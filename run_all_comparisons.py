...
# ------------------------------------------------------------------------------
def process_video_pair(video_path, gt_path, yolo_model, movenet_fn, mp_model_path, out_folder, limit_frames=None, save_video=True, save_plots=True):
    ...
    # Auto-detect Exercise ID and side
    exercise_id = 1
    ...
    # Side detection (first 15 frames)
    left_side_errors = []
    right_side_errors = []
    ...
    summary = {"video": os.path.basename(video_path), "exercise": f"{exercise_id:02d}", "side": selected_side}
...
- **ข้างที่วิเคราะห์:** {selected_side.upper()}
...
def main():
    ...
    for idx, (v_path, gt_path) in enumerate(pairs):
        ...
        print(f"[{idx+1}/{total_videos}] Processing: {parent_folder}/{video_name} ...")
        t0 = time.time()
        ...
        if res:
            ...
            results.append(res)
            dt = time.time() - t0
            print(f"      Success! Side: {res['side'].upper()} | MP MAE: {res['MediaPipe_mae']:.2f}° | YOLO MAE: {res['YOLOv8_mae']:.2f}° | MN MAE: {res['MoveNet_mae']:.2f}° | Time: {dt:.1f}s")
        else:
            print(f"      [WARN] Failed to process {parent_folder}/{video_name}")
    ...
    for _, r in df.iterrows():
        md_rows.append(
            f"| {r['folder']}/{r['video']} | {r['side'].upper()} | {r['YOLOv8_mae']:.2f}° | {r['MoveNet_mae']:.2f}° | {r['MediaPipe_mae']:.2f}° |"
        )
        
    joined_rows = "\n".join(md_rows)
    md_content = f"""# รายงานการเปรียบเทียบประสิทธิภาพแบบกลุ่ม (Batch Multi-Model Comparison Report)

รายงานการประเมินความคลาดเคลื่อนของการวัดมุมข้อต่อและการติดตามพิกัดข้อต่อของโมเดล **YOLOv8-pose**, **MoveNet**, และ **MediaPipe** เทียบกับข้อมูลอ้างอิงจริง **OptiTrack Ground Truth** ครอบคลุมกล้องและท่ากายภาพบำบัดทั้งหมด
...
| แหล่งวิดีโอ | ข้างที่ตรวจจับ | YOLOv8 MAE | MoveNet MAE | MediaPipe MAE |
| :--- | :---: | :---: | :---: | :---: |
| {joined_rows}
